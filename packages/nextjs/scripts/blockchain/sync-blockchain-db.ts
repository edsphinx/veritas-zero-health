...
const ESCROW_ADDRESS = CONTRACTS.researchEscrow.address as Address;
const ESCROW_ABI = CONTRACTS.researchEscrow.abi;
...
function getDefaultMilestones(escrowId: number) {
  const milestones = [
    ...
    },
    {
      escrowId,
      milestoneId: 1,
      milestoneType: MilestoneType.DataSubmission,
      ...
      if (!dryRun) {
        const newStudy = await prisma.study.create({
          data: {
            ...
            researcherAddress: studyDetails.researcher.toLowerCase(),
            status,
            // Required fields not available from old smart contracts
            sponsor: studyDetails.researcher.toLowerCase(), // Use researcher as sponsor for now
            maxParticipants: 100, // Default placeholder value
            chainId: CHAIN_ID,
  ...
  try {
    ...
    if (!dryRun) {
      ...
      const milestonesToCreate = chainMilestones.map((m: any, index: number) => {
        ...
        const milestoneTypeNum = Number(m[2] || m.milestoneType);
        ...
        // Convert smart contract enum index to MilestoneType enum value
        // Contract: 0=Enrollment, 1=DataSubmission, 2=FollowUpVisit, 3=StudyCompletion, 4=Custom
        const milestoneTypeMap: Record<number, MilestoneType> = {
          0: MilestoneType.Enrollment,
          1: MilestoneType.DataSubmission,
          2: MilestoneType.FollowUpVisit,
          3: MilestoneType.StudyCompletion,
          4: MilestoneType.Custom,
        };
        const milestoneType = milestoneTypeMap[milestoneTypeNum] || MilestoneType.Custom;
