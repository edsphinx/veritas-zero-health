...
export default function Home() {
  ...
  // TEMPORARY DEBUG
  console.log('[HOME] Full auth object:', auth);
  console.log('[HOME] User:', auth.user);
  console.log('[HOME] Role:', auth.user?.role);
  console.log('[HOME] Is authenticated:', auth.isAuthenticated);

  // Determine primary CTA based on user role
  const getPrimaryCTA = () => {
    if (!auth.isAuthenticated) {
      return { label: 'Get Started', href: '/patient' };
    }
  ...
  const handlePrimaryCTAClick = (e: React.MouseEvent) => {
    // If not authenticated and trying to access protected routes, show alert
    if (!auth.isAuthenticated && primaryCTA.href !== '/patient') {
      e.preventDefault();
      alert(
        ...
        <section className="relative overflow-hidden bg-gradient-to-b from-background via-muted/30 to-background py-24">
          <div className="container mx-auto px-4">
            ...
            >
              ...
              >
                <Badge variant="outline" className="mb-6 px-4 py-2 bg-primary/10 border-primary/20 inline-flex items-center gap-2">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-12">
                <Button
                  asChild
                  size="lg"
                  className="px-8 py-6 text-lg shadow-lg hover:shadow-xl group"
                >
